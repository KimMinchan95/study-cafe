...
function main() {
    ...
    if (workContent !== null && !hasValidContent(workContent)) {
        errors.push(
            `"✨ 작업 내용"에 구체적인 내용을 입력해 주세요. (불릿 항목에 설명 추가)`
        );
    }
    ...
    if (
        ...
        errors.push(
            `"🧠 구현 포인트"에 구체적인 내용을 입력해 주세요. (불릿 항목에 설명 추가)`
        );
    }

    const reviewContent = getSectionContent(body, '## ⚠️ 리뷰 포인트');
    if (reviewContent !== null && !hasValidContent(reviewContent)) {
        errors.push(
            `"⚠️ 리뷰 포인트"에 구체적인 내용을 입력해 주세요. (불릿 항목에 설명 추가)`
        );
    }
    ...
    if (checklistContent !== null && !CHECKBOX_CHECKED.test(checklistContent)) {
        errors.push(
            '"✅ 체크리스트"에서 확인한 항목을 하나 이상 체크해 주세요.'
        );
    }
